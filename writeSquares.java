...
prints the first n squares separated by commas, 
with the odd squares in descending order 
...
if passed a value less than 1. Note that the output does not advance onto the next line.
*/
...
    else{ //among rest
        if(n%2 == 1){ //odd
            ...
            writeSquares(n-2);
            System.out.print(", " + (int)Math.pow(n-1, 2));
        }
        else{ //even
            writeSquares(n-1);
            System.out.print(", " + (int)Math.pow(n, 2));
